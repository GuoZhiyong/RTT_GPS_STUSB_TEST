...
* Others:
***********************************************************/
static rt_err_t dev_gsm_init( rt_device_t dev )
{
...
* Others:
***********************************************************/
static rt_err_t dev_gsm_open( rt_device_t dev, rt_uint16_t oflag )
{
...
* Others:
***********************************************************/
static rt_err_t dev_gsm_close( rt_device_t dev )
{
...
* Others:
***********************************************************/
static rt_size_t dev_gsm_read( rt_device_t dev, rt_off_t pos, void* buff, rt_size_t count )
{
	return RT_EOK;
}
...
static rt_size_t dev_gsm_write( rt_device_t dev, rt_off_t pos, const void* buff, rt_size_t count )
{
	rt_size_t	len = count;
...
* Others:
***********************************************************/
static rt_err_t dev_gsm_control( rt_device_t dev, rt_uint8_t cmd, void *arg )
{
	switch( cmd )
	{
		case CTL_STATUS:
	...
	while( 1 )
	{
		...
		if( tts_playing == 0 )
		{
			...
			if( res == RT_EOK )
			{
				...
				dev_gsm_write( &dev_gsm, 0, ptts, strlen( ptts ) );
				tts_playing = 1;
				rt_free( ptts );
			}
		}
...
/*初始化串口接收缓冲区*/
	...
	dev_gsm.init	= dev_gsm_init;
	dev_gsm.open	= dev_gsm_open;
	dev_gsm.close	= dev_gsm_close;
	dev_gsm.read	= dev_gsm_read;
	dev_gsm.write	= dev_gsm_write;
	dev_gsm.control = dev_gsm_control;
...
{
	return dev_gsm_write( &dev_gsm, 0, sinfo, strlen( sinfo ) );
}
...
rt_size_t gsm_ipsend( uint8_t* buff, rt_size_t count, rt_int32_t timeout )
{
	...
	dev_gsm_write( &dev_gsm, 0, buf_start, strlen( buf_start ) );
	...
	dev_gsm_write( &dev_gsm, 0, buf_end, 3 );
